...
- **Provenance Protocol 0.2.0 — §2.1 covers transcription, not only first introduction.** Marking at Introduction-Time secured the moment an idea *enters* the working materials; the second moment — when a claim moves *within* them, from reading note to outline to draft to shared register — is at least as lossy and far more frequent. A target format earns its usefulness by making entries uniform, and that is precisely what strips the metadata: who claimed this, checked against what, was it ever in doubt. Afterwards a note that read *"the assistant summarised this, I have not read it"* is a sentence that says only what it says. Two properties make it worse than ordinary forgetting: the loss is **asymmetric** (material describing *itself* has a maintainer, material describing *something else* has no owner and so does not decay noticeably — it simply stops being true), and an unsupported reference is **indistinguishable from an outdated one** after the fact, so checking for currency never finds the pointer that was empty from the start. Three how-to-apply lines follow, including the one that matters most in practice: a target format with no slot for provenance is a format that launders it — add the slot, or do not move load-bearing claims into it. Promoted from the maintainer's practice corpus (July–August 2026), five documented cases across two working contexts and two genres, covering both directions of the same movement.
