# Intellectual-Hygiene Discipline
...
This blurring has consequences that other disciplines in this repository do not address. Source discipline tracks where *external sources* came from; it does not track where *the researcher's own working ideas* came from. Writing discipline tracks fidelity between source and final text; it does not track who first proposed a phrase or framework. Actant self-check tracks the *voice* of the asserting researcher; it does not track whether that voice's content was suggested by an assistant. Falsification discipline tests whether claims could be wrong; it does not test whether claims are the researcher's own.

Intellectual-hygiene discipline addresses the gap. Its purpose is threefold:
...
3. **Learning-progress verifiability** — when AI assistance is used, the question *"did I learn this, or am I scaffolded by it?"* must remain answerable. A discipline that tracks introduction-source enables retrospective verification: ideas the researcher used once and could not reproduce independently are still scaffolded; ideas the researcher has internalised and can reproduce, defend, and extend without re-prompting are learned. The same tracking enables verification of the opposite — apparent learning that on re-test turns out to be sustained AI dependency.

The discipline does not constrain *how* AI assistance is used; it constrains how the use is *recorded*. The recording is what makes the other three goals operational.
...
- For methodological moves, ADRs, and discipline-additions, name the originating side in the document body (*"This pattern was first surfaced by [the assistant / by me / jointly]"*).
...
**Why.** A scientific project spans weeks to years. Working sessions are episodic; the project is continuous. Markers that exist only in chat-transcripts or session-memory are not durable — chat services delete, memory rotates, contexts compact, exports get lost. A discipline that depends on transient records produces an audit-trail that is in principle but not in practice reconstructable. The discipline therefore requires *durable* markers in the project's own working materials: file commits, decision-log entries, glossary annotations, footnotes in working drafts.
...
**Rule.** The discipline distinguishes *scaffolded* use (the researcher applies an idea but cannot reproduce it without re-prompting the assistant) from *internalised* use (the researcher can reproduce, defend, and extend the idea independently). Tracking introduction-events plus subsequent independent-uses enables retrospective verification of which side a particular idea sits on.
...
- Failures to re-derive are not failures of the discipline. They are exactly the data the discipline exists to surface — the researcher now knows where their dependency sits and can either deepen the learning or accept the dependency knowingly.

---

## 3. Operationalisation in the Workflow

The discipline integrates into existing project workflows rather than requiring separate tooling:
...
- **At session-end:** append to the project-decision-log the load-bearing introductions of the session — concepts, terms, methodological moves, ADR-decisions, and discipline-additions. Note for each: originating side, brief seed-note.
...
The discipline does not require a separate file *if* the project's existing decision-log or methodology-log can carry the additional structure. For projects without such a file, a minimal `intellectual-hygiene-log.md` at the project root serves the purpose.

---

## 4. Failure-Mode Gallery

Three failure modes the discipline guards against, drawn from observed patterns in AI-assisted humanities work:
...
In all three cases, the failure is *recoverable* only with a pre-existing record. After-the-fact reconstruction is unreliable. The discipline's value is its before-the-fact discipline.

---

## 5. Relation to Other Disciplines

Intellectual-hygiene discipline is a *meta-level* discipline, operating across the full source-to-publication chain rather than at a single pipeline stage. Like falsification discipline, it tests an attribute of the methodology's outputs — in falsification's case *"could this claim be wrong?"*, in intellectual-hygiene's case *"where did this claim come from?"*. The two meta-disciplines are complementary axes of audit: falsification audits *validity*, intellectual-hygiene audits *provenance*.

- **Source discipline** tracks the provenance of *external sources*. Intellectual-hygiene tracks the provenance of *the researcher's own developing ideas*, including the AI's contributions to those ideas. The two disciplines are orthogonal; both are required.
- **Interpretive-frame discipline** tracks lexicon-stability across stage transitions. Intellectual-hygiene tracks attribution-stability across the same transitions, on a different axis (origin, not lemma).
- **Writing discipline** tracks fidelity between source and text. Intellectual-hygiene tracks fidelity between *who introduced an idea* and *who is credited for it*.
- **Actant self-check** audits the *voice* of the asserting researcher. Intellectual-hygiene audits whether that voice's content was the researcher's own — closely related, but distinct: voice-positionality can be intact while attribution-record is missing.
- **Tool discipline** is infrastructure-upstream. Intellectual-hygiene depends on it: if the tools used to record provenance are themselves unreliable, the record is unreliable.
- **Falsification discipline** tests *whether* commitments can be wrong. Intellectual-hygiene tests *whose* commitments they are. Both are meta-level.

---

## 6. What This Discipline Does **Not** Provide

- It does not prescribe how AI assistance should be used. The discipline operates on the *recording* of use, not the *amount* or *kind* of use. A researcher may rely heavily or lightly on AI; the discipline applies in both cases identically.
- It does not adjudicate attribution disputes. Where the originating side is genuinely ambiguous, the discipline requires the ambiguity to be *recorded* (category *co-developed* or explicit note); it does not provide a procedure for resolving the ambiguity into a clean attribution.
- It does not measure learning-progress quantitatively. The independent-deployment-test is qualitative; the discipline produces a *learned* or *scaffolded* judgement per concept, not a metric.
- It does not address authorship policies of specific venues. Different journals, conferences, and funding bodies have different AI-attribution rules; the discipline's records are *inputs* to those policies, not substitutes for them.
- It does not address the question of whether AI-introduced ideas are *valuable*. That is the falsification discipline's territory and the analyst's own intellectual judgement; the intellectual-hygiene discipline records the origin without evaluating the worth.

---

## 7. Cross-Refs

- [[source-discipline]] — external-source provenance (complementary axis).
- [[interpretive-frame-discipline]] — lexicon-stability across stage transitions (orthogonal axis).
- [[writing-discipline]] — composition-stage fidelity (different layer).
...
- Project-baseline attribution-hygiene rule (held in the project's cross-project instructions, in German) — the baseline rule from which this discipline expands. In English paraphrase: strict separation between the researcher's own knowledge or intuitions and AI-introduced sources; AI-introduced material must be explicitly marked and not retroactively attributed as prior knowledge.

---

*Versioning: working-draft. The discipline emerged 2026-05-21 as a methodology-lift from a user-stated requirement: tracking of human-introduced vs. AI-introduced ideas across a scientific project's span, to support IP-clarity, audit-traceability, and learning-progress verifiability. Per the auxiliary-hypothesis admissibility test (falsification-discipline §3 OT-3): the addition increases the methodology's forbidden set by three items — unrecorded provenance at introduction-time, unrecoverable audit-trails over project-span, and unverified learning-progress claims. Substantial structural re-articulation triggers a status update. Inline patch edits do not.*
