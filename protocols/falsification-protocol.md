...
- **(e) Yield declared before the run.** Before an empirical check starts, state what will exist after it — in **both** outcomes. An investigation whose null outcome yields nothing was designed badly, not run unluckily. Three parts: name the anchored goal the check can move (a status field, an open question, a declared gap — something that exists outside the investigator's head); state the yield of the best *and* the worst outcome, declaring **asymmetry** where it holds (an existence negation — *"no such standard exists"* — can be refuted by one find but never confirmed by any number of misses, so its null yield is a search record, not a finding); and identify the **cheapest partial step that could spare the expensive ones** — the positive control that shows the instrument bites, the sharpest single refuter — and run that first. The point is not to run less; it is to run in an order in which each step can change whether the next is needed. Empirical case (August 2026, the maintainer's practice): a six-axis literature sweep with full adversarial verification was launched on all axes at once, correctly pre-registered in every other respect; the two decisive axes — the positive control and the sole direct refuter — would have been a third of the cost, and the computed token floor was exceeded by a third while the run broke off mid-verification at a session limit. A staged design would plausibly have finished its decisive stage first; that is an observation from one case, not a proof. This clause matters more, not less, when the checking is delegated: an archive trip disciplines the investigator through its own effort, a delegated agent run does not — the economics of checking do not disappear with delegation, they become invisible.
...
**How to apply.**
- OT-8, OT-9, and OT-10 below operationalise clauses (a)–(c); clauses (d) and (e) fold into the pre-registration document itself. Clauses (a′) and (c′) extend the reach of (a) and (c) rather than adding machinery: (a′) applies wherever no pre-registration exists, (c′) wherever the arrangement — not the material — could account for the result.
- For (e), the ad-hoc case gets the cheap version, exactly as (a′) gives it for instrument scope: before any delegated check — a quick sweep, a batch query — say in one sentence what the empty result will mean and what the run costs. If the answer to the first is "nothing" and to the second is "a lot", the design is wrong, not the question.
